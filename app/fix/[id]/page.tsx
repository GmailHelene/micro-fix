  ...
  if (status === 'awaiting_changes') {
    return (
      <div className="mt-4 rounded-xl bg-orange-50 border border-orange-200 px-4 py-2.5 text-sm text-orange-700 font-medium">
        Endringer ønsket — se melding fra Micro-fix nedenfor og oppdater forespørselen.
      </div>
    );
  }
  return (
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        {fix.status === 'awaiting_changes' && fix.admin_note && (
          <div className="rounded-2xl bg-orange-50 border border-orange-200 p-5 mb-6">
            <p className="text-xs font-semibold text-orange-700 uppercase tracking-wider mb-2">Micro-fix ber om endringer</p>
                ...
                {messages.map(msg => (
                  <div key={msg.id} className={`flex ${msg.sender === 'customer' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm
                      ...
                      {msg.sender === 'admin' && <p className="text-xs font-semibold text-slate-500 mb-1">Micro-fix</p>}
                      <p>{msg.content}</p>
