...
import {
  ArrowLeft, Key, Mail, Lock, Check, X, CornerUpLeft, Play,
...
export default function AdminFixDetailPage() {
  ...
  if (!fix) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center">
      <button onClick={() => router.push('/admin/dashboard')} className="text-blue-600 text-sm hover:underline inline-flex items-center gap-1"><ArrowLeft className="w-4 h-4" /> Tilbake</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <button onClick={() => router.push('/admin/dashboard')} className="text-sm text-slate-500 hover:text-slate-800 mb-8 inline-flex items-center gap-1">
          <ArrowLeft className="w-4 h-4" /> Oppdragsoversikt
        </button>
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          ...
          <div className="space-y-5">
            ...
            {fix.access_info && (
              <div className="rounded-3xl bg-amber-50 border border-amber-200 p-5 shadow-sm">
                <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-2 inline-flex items-center gap-1"><Key className="w-3.5 h-3.5" /> Tilgangsinformasjon fra kunde</p>
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {['pending_approval', 'awaiting_offer_approval'].includes(fix.status) && (
                  <button
                    ...
                    disabled={working}
                    className="w-full rounded-xl bg-emerald-600 text-white px-4 py-3 text-sm font-semibold hover:bg-emerald-700 disabled:opacity-40 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
                    <Check className="w-4 h-4" /> Godkjenn — standard pris
                  </button>
                )}
                ...
                {['pending_approval', 'awaiting_changes', 'awaiting_offer_approval'].includes(fix.status) && (
                  <>
                    <button
                      ...
                      className="w-full rounded-xl border border-purple-200 bg-purple-50 text-purple-700 px-4 py-2.5 text-sm font-semibold hover:bg-purple-100 transition-colors inline-flex items-center justify-center gap-1.5"
                    >
                      <MessageSquare className="w-4 h-4" /> Send custom tilbud med ny pris
                    </button>
                    {showOfferInput && (
                ...
                {['pending_approval', 'awaiting_changes', 'awaiting_offer_approval'].includes(fix.status) && (
                  <>
                    <button
                      ...
                      className="w-full rounded-xl border border-orange-200 bg-orange-50 text-orange-700 px-4 py-2.5 text-sm font-semibold hover:bg-orange-100 transition-colors inline-flex items-center justify-center gap-1.5"
                    >
                      <CornerUpLeft className="w-4 h-4" /> Be om endringer
                    </button>
                    {showNoteInput && (
                      <div className="space-y-2">
                        <textarea
                ...
                {['pending_approval', 'awaiting_changes', 'awaiting_offer_approval'].includes(fix.status) && (
                  <button
                    ...
                    disabled={working}
                    className="w-full rounded-xl border border-red-200 bg-red-50 text-red-700 px-4 py-2.5 text-sm font-semibold hover:bg-red-100 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
                    <X className="w-4 h-4" /> Avvis forespørsel
                  </button>
                )}
                ...
                {fix.status === 'awaiting_payment' && fix.payment_status === 'paid' && (
                  <button
                    ...
                    disabled={working}
                    className="w-full rounded-xl bg-indigo-600 text-white px-4 py-3 text-sm font-semibold hover:bg-indigo-700 disabled:opacity-40 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
                    <Play className="w-4 h-4" /> Start arbeid
                  </button>
                )}
                ...
                {fix.status === 'in_progress' && (
                  <button
                    ...
                    className="w-full rounded-xl bg-emerald-600 text-white px-4 py-3 text-sm font-semibold hover:bg-emerald-700 disabled:opacity-40 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
                    <Check className="w-4 h-4" /> Marker fullført
                  </button>
                )}
                ...
                <div className="border-t border-slate-100 pt-3 mt-1">
                  <button
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 text-slate-600 px-4 py-2 text-xs font-medium hover:bg-slate-100 disabled:opacity-40 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
                    <Mail className="w-3.5 h-3.5" /> Send status-e-post til kunde på nytt
                  </button>
              ...
              <div className="rounded-3xl bg-blue-50 border border-blue-200 p-5 space-y-4">
                ...
                {!fix.custom_payment_url && (
                  <div className="rounded-xl bg-white border border-blue-100 px-4 py-3">
                    <p className="text-xs font-semibold text-emerald-700 mb-1 inline-flex items-center gap-1"><Check className="w-3.5 h-3.5" /> Ingen handling kreves</p>
                ...
                <div>
                  ...
                  {fix.custom_payment_url && (
                    <div className="mt-2 space-y-1.5">
                      <p className="text-xs text-emerald-600 inline-flex items-center gap-1"><Check className="w-3.5 h-3.5" /> Custom lenke er satt — kunden bruker denne ved betaling</p>
                      {fix.payment_status !== 'paid' && fix.payment_status !== 'authorized' && (
                        <div className="rounded-xl bg-amber-50 border border-amber-200 px-3 py-2.5 text-xs text-amber-800">
                          <p className="font-semibold mb-1 inline-flex items-center gap-1"><AlertTriangle className="w-3.5 h-3.5" /> Manuell bekreftelse kreves</p>
                          ...
                          <button
                            ...
                            className="rounded-lg bg-amber-600 text-white px-3 py-1.5 text-xs font-semibold hover:bg-amber-700 disabled:opacity-40 transition-colors inline-flex items-center gap-1"
                          >
                            <Check className="w-3.5 h-3.5" /> Bekreft betaling mottatt
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <div className="rounded-3xl bg-amber-50 border border-amber-200 p-5">
              <h3 className="text-sm font-semibold text-amber-800 mb-1 inline-flex items-center gap-1"><Lock className="w-3.5 h-3.5" /> Interne notater</h3>
                ...
                <div className="flex justify-between"><span className="text-slate-500">Betaling</span>
                  <span className={`font-medium ${
                    ...
                    {fix.payment_status === 'authorized'     ? 'Reservert' :
                     fix.payment_status === 'capture_failed' ? 'Capture feilet' :
